(() => {
  ...
  var GraphTracer = class {
    ...
    error(error2, req, middleware, context = null) {
      ...
      this.store.saveSpan(span);
      const message = error2?.message || "Middleware failed";
  ...
  async function fetchWellknownOauthAuthorizationServer(issuer, client2) {
    let res = client2.get(src_default3.url(issuer, ".wellknown/oauth_authorization_server"));
    if (res.ok) {
    ...
    throw src_default3.metroError("metro.oidcmw: Error while fetching " + issuer + ".wellknown/oauth_authorization_server", res);
  }
